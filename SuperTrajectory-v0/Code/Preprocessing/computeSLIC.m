...
function SLIC = computeSLIC( options, shot )
    ...
    SLIC{ left } = SLICGeneration( options, startFrame, frames, left );
    options.isLeft = 0;
    SLIC{ right } = SLICGeneration( options, startFrame, frames, right );

    
    
end

function SLIC = SLICGeneration( options, startFrame, frames, stereo )
% generate the SLIC method for each stereo video
    ...
    if( options.vocal )
        if ( options.isLeft )
            ...
            reminder = '(Right Video)';
        end
        fprintf( 'computeSLIC: %s Total time taken: %.2f sec\n', reminder, totalTimeTaken );
