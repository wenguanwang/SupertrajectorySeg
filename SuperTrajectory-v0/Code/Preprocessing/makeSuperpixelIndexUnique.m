...
function [ output, frameSuperpixelBounds, globalIDs] = ...
    makeSuperpixelIndexUnique( superpixelsLabel )

    output = cell( size( superpixelsLabel ) );

    globalIDs = 0;
    frameSuperpixelBounds = [ 1 ];
    for( frame = 1: length( superpixelsLabel ) )      
        indexes = uint32( max( superpixelsLabel{ frame }(:) ) );
        output{ frame } = globalIDs + uint32( superpixelsLabel{ frame } );
