...
function [ flow, backflow ] = computeOpticalFlow( options, shot )
...
%         filename = fullfile( flowfolder, sprintf( 'flowShot%i.mat', shot ) );
%     else
%         fprintf( 'computeBackwardOpticalFlow: Processing shot #%i\n', shot );
%         filename = fullfile( flowfolder, sprintf( 'flowBackwardShot%i.mat', shot ) );
%     end
    ...
    if( exist( filename, 'file' ) )
        ...
        if( strcmp( options.flowmethod, 'broxPAMI2011' ) )
            [ flow, backflow ] = computeBroxPAMI2011Flow( options, shot );
        elseif( strcmp( options.flowmethod, 'sundaramECCV2010' ) )
            [ flow, backflow ] = computeSundaramECCV2010Flow( options, shot );
        else
            error( 'Unknown flow computation method' )
        end
        
        save( filename, 'flow', 'backflow', '-v7.3' );
    end
