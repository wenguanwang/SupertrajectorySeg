  ...
  int nOrients, int softBin, bool full, float clip )
{
  ...
  hogChannels( H, R, N, hb, wb, nOrients, clip, 1 );//hogChannels( H, R, N, hb, wb, nOrients, clip, 0 );
  wrFree(N); wrFree(R);
}
...
void mGradHist( int nl, mxArray *pl[], int nr, const mxArray *pr[] ) {
  ...
  nChns = useHog== 0 ? nOrients : (useHog==1 ? nOrients : nOrients*3+5);/////nChns = useHog== 0 ? nOrients : (useHog==1 ? nOrients*4 : nOrients*3+5);
